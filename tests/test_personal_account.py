...
class TestPersonalAccount:
    def register_and_login(self, driver, wait, test_user):
        driver.get(Urls.REGISTER_URL)
        wait.until(EC.visibility_of_element_located(AuthPageLocators.NAME_INPUT)).send_keys(test_user["name"])
        driver.find_element(*AuthPageLocators.EMAIL_INPUT).send_keys(test_user["email"])
        ...
        try:
            wait.until(EC.visibility_of_element_located(AuthPageLocators.LOGIN_BUTTON_FINAL))
        except:
        ...
        driver.find_element(*AuthPageLocators.PASSWORD_INPUT).send_keys(test_user["password"])
        driver.find_element(*AuthPageLocators.LOGIN_BUTTON_FINAL).click()
    ...
    def test_go_to_personal_account(self, driver, wait, test_user):
        self.register_and_login(driver, wait, test_user)
        wait.until(EC.element_to_be_clickable(MainPageLocators.PERSONAL_ACCOUNT_BUTTON)).click()
    ...
    def test_logout_from_account(self, driver, wait, test_user):
        ...
        assert wait.until(EC.visibility_of_element_located(AuthPageLocators.LOGIN_BUTTON_FINAL)).is_displayed()
